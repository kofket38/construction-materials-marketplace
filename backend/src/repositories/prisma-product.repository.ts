...
export class PrismaProductRepository implements ProductRepository {
  ...
  async findImages(productId: string): Promise<ProductImageEntity[]> {
    const isSeededProduct = SEEDED_PRODUCT_IDS.has(productId);

    if (isSeededProduct) {
      const primaryImage = await this.client.productImage.findFirst({
        where: { productId, isPrimary: true },
        orderBy: [{ createdAt: "asc" }, { id: "asc" }],
      });

      return primaryImage ? [mapProductImage(primaryImage)] : [];
    }
